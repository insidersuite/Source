    ...
    class LoginController extends Controller
    {
        ...
        public function facebook_login_callback() {
            ...
            if ($check->email != NULL) {
                ...
                Mail::send('mail_templates.visitor_email', ['username' => $new_user->username], function ($message) use ($email) {
                           $message->to($email)->subject('🎉Your Insider Suite membership is here!');
                           });
                $email_contact = \Config::get('mail.email_contact');
                Mail::send('mail_templates.signup', ['username' => $new_user->username], function ($message) use ($email_contact) {
                           $message->to($email_contact)->subject('New User Signup');
                           });
                // Mail::send('mail_templates.signup', ['username' => $new_user->username], function ($message) use ($email) {
                //     $message->to($email)->subject('New User Signup');
                // });
                //$this->send_mail('contact@insidersuite.com',['username' => $new_user->username],'New User Signup','signup');
                //$this->send_mail($new_user->email,['username' => $new_user->username],'New User Signup','signup');
