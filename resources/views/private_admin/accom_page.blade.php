					...
					<div class="content-box">						
						<div class="header-list">
							<div class="row">
                                ...
                                <div class="label-table min_day_booking">Min stay</div>
                                <div class="label-table status">Status</div>
                                <div class="label-table sort">Sort</div>
							</div>
						</div>
						<div class="item-list">
							@if ($count == 0) 
							...
							<div class="item">
								<div class="row">
                                    <div class="label-table city">{{$accom->city_id}} <a class="edit" data-id="{{$accom->id}}">Edit</a>
                                     <!-- <a class="delete" data-id="{{$accom->id}}">Delete</a> -->
                                 </div>
									<div class="label-table name">{{$accom->name}}</div>
									<div class="label-table address">{{$accom->full_address}}</div>
                                    <div class="label-table min_day_booking">{{$accom->min_day_booking}}</div>
                                    <div class="label-table status">{{$accom->status}}</div>
                                    <div class="label-table sort">{{$accom->timestamp}}</div>
								</div>
							</div>
							@endforeach
							...
							{{$accoms->appends(['id' => $current_id])->links()}}   
							<!-- fixed end --> 
